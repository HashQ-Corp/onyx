...
const getSalesforceOAuthUrlSS = async (nextUrl: string | null): Promise<string> => {
  const url = UrlBuilder.fromInternalUrl("/auth/salesforce/authorize");
  if (nextUrl) {
    url.addParam("next", nextUrl);
  }
...
export const getAuthUrlSS = async (
  ...
  switch (authType) {
    ...
    case "salesforce_oauth": {
      return await getSalesforceOAuthUrlSS(nextUrl);
    }
    case "cloud": {
      return await getGoogleOAuthUrlSS(nextUrl);
    }
